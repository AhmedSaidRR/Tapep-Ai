...
def _get_vectorstore() -> PineconeVectorStore:
    ...
    """
    logger.info("🔗 [MediBlaze] Initialising Pinecone vectorstore (cached)…")
    ...
    logger.info("✅ [MediBlaze] Pinecone vectorstore ready.")
    return vectorstore
...
def rag_tool(query: str) -> str:
    ...
    """
    try:
        logger.info(f"📖 [MediBlaze] RAG search: {query}")
        ...
        _rag_instance = create_retriever_tool(
            ...
            (
                "🏥 Searches the MediBlaze medical knowledge base for comprehensive "
        ...
        if not result or len(str(result).strip()) < 20:
            logger.warning("⚠️ [MediBlaze] RAG returned no useful results.")
            return (
        ...
        logger.info("✅ [MediBlaze] RAG search completed successfully.")
        return f"**📚 From MediBlaze Health Knowledge Base:**\n\n{result}"

    except Exception as e:
        logger.error(f"❌ [MediBlaze] RAG error: {str(e)}")
        return (
...
def medical_web_search(query: str) -> str:
    ...
    """
    try:
        logger.info(f"🔍 [MediBlaze] Web search: {query}")
        ...
        if not results or len(str(results).strip()) < 20:
            logger.warning("⚠️ [MediBlaze] Web search returned no relevant results.")
            return (
        ...
        logger.info("✅ [MediBlaze] Web search completed successfully.")
        return (
        ...
        logger.error(f"❌ [MediBlaze] Web search error: {str(e)}")
        return (
